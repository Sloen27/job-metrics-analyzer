...
export const METRIC_DEFINITIONS: MetricDefinition[] = [
  ...
  // TelegramProcessingJob метрики
  { key: 'totalNewPosts', label: 'Новых постов', unit: '', format: 'number', higherIsBetter: true },
  { key: 'totalAllPosts', label: 'Всего постов', unit: '', format: 'number', higherIsBetter: true },
  { key: 'avgActiveSources', label: 'Активных источников', unit: '', format: 'number', higherIsBetter: true },
  { key: 'avgProcessedSources', label: 'Обработанных источников', unit: '', format: 'number', higherIsBetter: true },
  { key: 'avgConnectedSources', label: 'Подключённых источников', unit: '', format: 'number', higherIsBetter: true },
  { key: 'avgUsedAccounts', label: 'Использованных аккаунтов', unit: '', format: 'number', higherIsBetter: true },
...
function aggregateDailyMetrics(dayJobs: JobLog[]): AggregatedJobMetrics {
  ...
  const allPosts = dayJobs.filter(j => j.metrics.totalPosts !== undefined);
  if (allPosts.length > 0) {
    metrics.totalAllPosts = allPosts.reduce((sum, j) => sum + (j.metrics.totalPosts || 0), 0);
  }
  ...
  const connectedSources = dayJobs.filter(j => j.metrics.connectedSources !== undefined);
  if (connectedSources.length > 0) {
    metrics.avgConnectedSources = connectedSources.reduce((sum, j) => sum + (j.metrics.connectedSources || 0), 0) / connectedSources.length;
  }
  
  const usedAccounts = dayJobs.filter(j => j.metrics.usedAccounts !== undefined);
  if (usedAccounts.length > 0) {
    metrics.avgUsedAccounts = usedAccounts.reduce((sum, j) => sum + (j.metrics.usedAccounts || 0), 0) / usedAccounts.length;
  }
...
function getAvailableMetrics(dailyMetrics: DailyMetrics[]): string[] {
  ...
  for (const day of dailyMetrics) {
    ...
    if (day.metrics.totalAllPosts) available.add('totalAllPosts');
    if (day.metrics.avgActiveSources) available.add('avgActiveSources');
    if (day.metrics.avgProcessedSources) available.add('avgProcessedSources');
    if (day.metrics.avgConnectedSources) available.add('avgConnectedSources');
    if (day.metrics.avgUsedAccounts) available.add('avgUsedAccounts');
    // Проценты
